...
const nav = [
  ...
  { label: "نمط التصميم", path: "/design-system", icon: "solar:palette-round-bold-duotone" },
];
...
export function Sidebar({ collapsed, mobileOpen, onClose }: { collapsed: boolean; mobileOpen: boolean; onClose: () => void }) {
  ...
  return <aside className={`app-sidebar ${isCollapsed ? "is-collapsed" : ""} ${mobileOpen ? "is-mobile-open" : ""}`} aria-label="Primary navigation">
    <div className="brand-lockup"><div className="brand-mark"><Icon icon="solar:layers-minimalistic-bold-duotone" size={20} /></div><div className="brand-copy"><div className="brand-name">Reactor قؤاد</div><div className="brand-subtitle">قلب التصميم</div></div></div>
    <nav className="sidebar-nav">
      ...
      <div className="nav-group-label" style={{ marginBlockStart: 25 }}>مراجع</div>
